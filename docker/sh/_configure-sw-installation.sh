...
docker-compose run --rm tools bash -c 'cd /var/www/shopware && php bin/console sw:firstrunwizard:disable'
# Preset API key
...
docker-compose run --rm tools mysql -u root -ptoor -h mysql shopware -e 'UPDATE s_core_config_elements SET value = "b:0;" WHERE name = "show_cookie_note"'

docker-compose run --rm tools bash -c 'cd /var/www/shopware && php bin/console sw:cache:clear'
