...
PACKAGE_NAME="*_update_*_latest.zip"
ENV_TESTS="../tests/.env"
...
docker-compose run tools find /source -maxdepth 1 -name "${PACKAGE_NAME}" -exec unzip -oq {} -d /var/www/shopware \;
...
echo "Run Mink"
docker-compose run tools ./behat --format=pretty --out=std --format=junit --out=/logs/mink --tags '@updater&&~@knownFailing'
