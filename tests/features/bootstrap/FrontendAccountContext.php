...
class FrontendAccountContext extends SubContext
{
    ...
    {
        $pageInfo = Helper::getPageInfo($this, ['controller']);
        ...
        /** @var \Shopware\Tests\Mink\Page\Frontend\Account|\Shopware\Tests\Mink\Page\Frontend\CheckoutConfirm $page */
    ...
    {
        $pageInfo = Helper::getPageInfo($this->getPage('CheckoutConfirm'), ['controller']);
        $pageName = ucfirst($pageInfo['controller']);
        ...
        $page->openAddressChangeForm();
        $this->waitForText('Adresse bearbeiten');
        $page->changeAddress($data);
        $this->waitForModalOverlayClosed();
    }
     ...
     * @When /^I change the payment method to (?P<paymentId>\d+):$/
     */
    public function iChangeThePaymentMethodTo($method, TableNode $table = null)
    {
        $pageInfo = Helper::getPageInfo($this->getPage('Account'), ['controller', 'action']);
        $pageName = ucfirst($pageInfo['controller']);
        ...
        if (!is_numeric($method)) {
            $method = $this->getMethodId('payment', $method);
        }
        ...
        sleep(8);
        $page->changePaymentMethod($data, $table);
        $this->waitForModalOverlayClosed();
    }
    ...
    {
        /** @var \Shopware\Tests\Mink\Page\Frontend\Account $page */
        $page = $this->getPage("Account");
    ...
    {
        /** @var \Shopware\Tests\Mink\Page\Frontend\Address $page */
        $page = $this->getPage('Address');
        $data = $table->getHash();
    ...
    {
        /** @var \Shopware\Tests\Mink\Page\Frontend\Account|\Shopware\Tests\Mink\Page\Frontend\AddressEdit $page */
    ...
    {
        /** @var \Shopware\Tests\Mink\Page\Frontend\Account|\Shopware\Tests\Mink\Page\Frontend\Address $page */
        $page = $this->getPage('Address');
        ...
        foreach ($addressManagementAddressBoxes as $box) {
            ...
            /** @var \Shopware\Tests\Mink\Page\Frontend\AddressDelete $page */
