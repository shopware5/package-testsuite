...
Feature: Checkout articles (scenario origin is account without articles in basket)
  ...
  Background: I can login as a user with correct credentials
    Given the following customer Accounts exist:
      | email                                                | group |
      | frontend-checkout-account-private@example.localhost  |       |
      | frontend-checkout-account-business@example.localhost | H     |
      | test@example.com                                     |       |
      | mustermann@b2b.de                                    | H     |
  ...
  Scenario Outline: I can login, add articles to basket and finish my order
    ...
    Examples:
      ...
      | test@example.com  | 15,60 € | 3,90 €   | 19,50 € | 16,39 €       | 3,11 € |
  ...
  Scenario Outline: I can register, add articles to basket and finish my order
    ...
    When  I put the current article "3" times into the basket
    Then  the cart should contain 1 articles with a value of "134,97 €"
    And   the aggregations should look like this:
      | label         | value    |
      | sum           | 134,97 € |
      | shipping      | 3,90 €   |
      | total         | 138,87 € |
      | sumWithoutVat | 116,70 € |
      | 19 %          | 22,17 €  |
