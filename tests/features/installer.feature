...
Feature: I can start and complete the installation process
  ...
  Scenario: I can execute the installation process
    ...
    Then the following "databaseForm" form fields must be required:
      | fieldname         |
      | c_database_user   |
      | c_database_schema |
    ...
    Then I should see "Datenbank Update wurde erfolgreich durchgeführt" after import is finished
    ...
    Then the following "shopBasicConfiguration" form fields must be required:
      | fieldname               |
      | c_config_shopName       |
      | c_config_mail           |
      | c_config_admin_name     |
